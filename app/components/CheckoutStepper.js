...
export default function CheckoutStepper({ currentStep }) {
  ...
  return (
    <div className="bg-white py-5 px-4 w-full max-w-[800px] mx-auto border-b border-gray-100">
      <div className="flex justify-between items-start relative max-w-[400px] mx-auto mt-2">
        {/* Connecting line */}
        ...
        {steps.map((step) => {
          ...
          return (
            <div key={step.id} className="flex flex-col items-center gap-2 z-10 w-1/4">
              ...
              <span className={`text-[12px] font-medium ${isActive || isPassed ? 'text-gray-800' : 'text-[#c0c0c0]'}`}>
                {step.label}
              </span>
            </div>
          );
